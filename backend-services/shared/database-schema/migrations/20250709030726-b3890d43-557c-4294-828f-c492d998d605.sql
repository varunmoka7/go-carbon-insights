-- Create industry taxonomy core schema
...
CREATE TABLE public.company_industries (
    ...
    relationship_type TEXT NOT NULL DEFAULT 'primary', -- 'primary', 'secondary', 'supplier'
...
CREATE TABLE public.industry_tags (
    ...
    tag_category TEXT NOT NULL DEFAULT 'general', -- 'scope3', 'reporting', 'regulation', 'general'
    description TEXT,
    color_code TEXT,
    created_at TIMESTAMP WITH TIME ZONE NOT NULL DEFAULT now()
);
...
CREATE TABLE public.archetype_configurations (
    ...
    key_scope3_categories TEXT[], -- Array of most relevant Scope 3 categories
...
-- Create RLS policies
-- Industry taxonomy - public read, admin write
...
-- Company industries - users can view their companies, admins can manage all
...
-- Industry tags - public read, admin write
...
-- Tag assignments - public read, admin write
...
-- Archetype configurations - public read, admin write
...
-- Taxonomy audit log - restricted access
...
-- Function to get industry by company
...
-- Function to get companies by industry archetype
...
-- Function to validate company industry allocations
...
BEGIN
    -- Get total allocation for primary industries
...
-- Create trigger function for taxonomy audit logging
...
-- Create some initial industry tags
